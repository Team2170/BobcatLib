...
public class SimDriveMotor implements DriveWrapper {
  ...
  public ModuleLimitsJson limits;
    public SimDriveMotor(ModuleConstants chosenModule, ModuleLimitsJson limits) {
        ...
        driveSim = new DCMotorSim(
                LinearSystemId.createDCMotorSystem(
                        DRIVE_GEARBOX, 0.01, chosenModule.driveGearRatio),
                DRIVE_GEARBOX);
    }
    ...
    public void setSpeed(SwerveModuleState desiredState, boolean isOpenLoop) {
        ...
        if (!isOpenLoop) {
            double rotationsPerSeconds = Conversions.MPSToRPS(desiredState.speedMetersPerSecond, chosenModule.wheelCircumference);
            ...
            driveFFVolts = DRIVE_KS * Math.signum(angularVelocityRadPerSec) + DRIVE_KV * angularVelocityRadPerSec;
    ...
    public double getPosition() {
        double pos = driveSim.getAngularPosition().baseUnitMagnitude();
        return pos;
    }
